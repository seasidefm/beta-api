...
async function main() {
    ...
    parsed.forEach((entry, index) => {
        ...
        if (!entry.songs) {
            throw new Error(`Malformed entry parsed[${index}]. No 'songs' key!`)
        }

        entry.songs.forEach((s: Record<string, any>, i: number) => {
            if (!s.song) {
                throw new Error(`Malformed entry parsed[${index}][${i}]. No 'song' key!`)
            }

            if (!s.date) {
                throw new Error(`Malformed entry parsed[${index}][${i}]. No 'date' key!`)
            }
        })
    })
    console.log('...OK')
    ...
    for (const entry of data) {
        ...
        for (const song of songs) {
            ...
            const createdSong = await s.findOrCreate(createdArtist.id, songTitle.trim())

            console.log(`Importing favorite > ${createdUser.name} -> ${createdSong.song_title}`)
            await f.create(createdUser.id, createdSong.id, new Date(song.date * 1000))
        }
    }
    console.log('...OK')
}
